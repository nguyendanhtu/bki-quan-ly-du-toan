...
*/
	...
	public static void F350ExportExcel(DateTime ip_dat_tu_ngay, DateTime ip_dat_den_ngay, decimal ip_dc_id_don_vi)
	{
		HttpContext.Current.Response.Clear();
		//Response.Buffer = true;
		HttpContext.Current.Response.AddHeader("content-disposition", "attachment;filename=" + "BaoCaoTinhHinhGiaiNganCacNguonVon.xls");
		...
		string v_str_result = F350Grid.RenderToString(ip_dat_tu_ngay, ip_dat_den_ngay, ip_dc_id_don_vi);
		//string style = @"<style> .textmode { mso-number-format:\@; } </style>";
		//HttpContext.Current.Response.Write(style);
		HttpContext.Current.Response.Output.Write(v_str_result);
		//Response.Write(sw.ToString());
