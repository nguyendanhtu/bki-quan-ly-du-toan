...
var PL01 = {
	...
	formatInputMoneyInit: function () {
		...
		for (var i = 0; i < lst_SoXetDuyet.length; i++) {
			$(lst_SoXetDuyet[i]).val(format_input($(lst_SoXetDuyet[i]).val().split(',').join('').split('.').join('')));
		}
		for (var i = 0; i < lst_SoBaoCao.length; i++) {
			$(lst_SoBaoCao[i]).val(format_input($(lst_SoBaoCao[i]).val().split(',').join('').split('.').join('')));
		}
		...
		if (strClass != '') {
			$('.' + classCongThuc + '.' + strClass).bind("keypress keyup keydown change", (function (e) {
				$('.' + classCongThuc + '.' + ma_so_parent)
					.val(format_input(PL01.computedValueByFormula(lstFormula, classCongThuc)));
